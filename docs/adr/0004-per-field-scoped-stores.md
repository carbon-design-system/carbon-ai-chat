...
**C. Keep `getState()` as a bundled getter composed over the stores — rejected.** The stores become the read model, and `getState()` survives as a deprecated convenience that assembles them. Nothing breaks, and the migration is optional forever — which is exactly the problem. No release after 2.0.0 takes breaking changes, so a getter that survives the major is not deprecated, it is permanent: two read models, forever, with the docs explaining when each is right — the question this record exists to answer once. The bundle also stops being reproducible: two of its fields are framework bookkeeping that get no store, so the getter could not return today's shape anyway. Against that stands the real price of removal, and it is small. Most `getState()` call sites in this repo's own demo and examples are one-shot reads in click handlers and render paths, and each becomes a mechanical store read at the major. This record takes that break knowingly.
