...
**The outbound queue advances on different rules.** A queue item marked `isStreaming` suppresses `moveToNextQueueItem` until the terminal chunk arrives (`chat/services/MessageService.ts:306-316`). A turn that never sets it advances as soon as the send settles (`:308`). The flag is armed only by `isStreamPartialItem(chunk)` (`ChatActionsImpl.ts:1175`), so this is not even a clean split by API. A chunk turn built entirely from `complete_item` chunks holds nothing and behaves like an upsert turn. Which queue semantics a host gets depends on the shape of its chunks.

**The stop button clobbers in one direction.** The chunk path's reset runs globally, with no message argument (`ChatActionsImpl.ts:1577-1584`, resetting at `:1582`, into `chat/utils/streamingUtils.ts:168`). A chunk turn finishing therefore hides a button an upsert turn raised. The reverse cannot happen. The upsert pipeline has no reset of its own, and the resets that could race a live chunk stream are guarded on `streamingMessageID` (`MessageService.ts:311`, `ChatActionsImpl.ts:1797`), so they return early while one is running. The remaining unguarded resets — restart, cancellation, and the error path (`ChatActionsImpl.ts:2228`, `MessageService.ts:796`, `OutboundMessageCoordinator.ts:107` and `:200`) — are not reachable from the upsert delivery path, which is what keeps the asymmetry one-directional. The bug reproduces in one ordering only, which is why it survived.
