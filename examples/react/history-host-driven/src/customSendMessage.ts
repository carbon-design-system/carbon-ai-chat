...
/**
 * Mock backend for the history example.
 *
 * Demonstrates: a client-side `customSendMessage` implementation that returns
 * either a static text response or a fake streamed response, so the example
 * has interactive turns alongside its rehydrated history payload.
 *
 * APIs exercised:
...
async function doFakeTextStreaming(
  ...
  // The host AbortSignal fires for both the stop button and a conversation reset, so any scheduled chunk emissions must be torn down to avoid pushing chunks into a stale or missing response.
  const abortHandler = () => {
    isCanceled = true;
  ...
  try {
    ...
    if (!isCanceled) {
      ...
      // Emit a truncated complete_item flagged with stream_stopped so the UI knows the response was halted instead of finished.
      const completeItem = {
        response_type: MessageResponseTypes.TEXT,
...
// Replace with a real production implementation that calls your conversational backend.
