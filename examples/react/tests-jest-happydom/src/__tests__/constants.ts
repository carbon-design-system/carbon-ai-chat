...
/**
 * Shared test constants for the happy-dom Jest harness: timeouts tuned for
 * happy-dom's slower shadow-DOM upgrade, plus deterministic message fixtures
 * (markdown with code + table, conversational-search payload) used by tests
 * that exercise lazy-loaded runtimes through the runner. Consumers import
