...
export async function sendUserMessage(
  ...
  // Carbon's input listens for the native `input` event rather than
  // React synthetic events, so we set value + dispatch instead of using
  // fireEvent.change(); bubbles:true lets the event escape the shadow root.
  await act(async () => {
    sendButton.click();
  });
