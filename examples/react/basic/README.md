...
Minimal React integration of `@carbon/ai-chat` that wires a mock backend,
listens to bus events, and renders a user-defined response.
...
- Surfacing feedback submissions to the page via a
  `FeedbackInteractionType.SUBMITTED` handler.
...
- You're exploring how `renderUserDefinedResponse` and `customSendMessage` fit
  together.
