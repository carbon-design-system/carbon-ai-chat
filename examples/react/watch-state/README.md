...
- Subscribing to `BusEventType.STATE_CHANGE` and reacting to changes in
  `homeScreenState.isHomeScreenOpen` and `activeResponseId`.
- Rendering the current view ("Homescreen" vs "Chat View") outside the chat UI.
- Passing `activeResponseId` into a `renderUserDefinedResponseFactory` so
  user-defined responses can adapt to active-response state.
