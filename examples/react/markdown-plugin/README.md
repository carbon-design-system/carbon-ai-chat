...
- Passing a `markdownItPlugins` array through the `markdown` prop to register a
  markdown-it plugin alongside the chat's built-ins.
- Memoizing the plugins array with `useMemo` so the markdown-it instance is not
  rebuilt every render.
- Rendering plugin output (KaTeX HTML) styled by a consumer-supplied stylesheet
