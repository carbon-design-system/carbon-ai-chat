...
/**
 * Custom history panel renderer for the chat-history-float example.
 *
 * Demonstrates: assembling the `@carbon/ai-chat-components` history primitives
 * (`HistoryShell`, `HistoryHeader`, `HistoryToolbar`, `HistoryPanel`, etc.)
 * into a working conversation picker that drives chat selection, pin/unpin,
 * rename, delete, and search through the parent `ChatInstance`.
 *
 * APIs exercised:
 *   - `ChatInstance.customPanels.getPanel`
 *   - `PanelType.HISTORY`
 *   - `HistoryShell`, `HistoryHeader`, `HistoryToolbar`, `HistoryContent`
 *   - `HistoryPanel`, `HistoryPanelMenu`, `HistoryPanelItem`,
 *     `HistoryPanelItems`, `HistorySearchItem`, `HistoryDeletePanel`
...
function ChatHistoryExample({
  ...
  // chat to close its built-in `PanelType.HISTORY` so launcher state and
  // focus return to the conversation surface.
  ...
  const handleSelectChat = useCallback(
    (event: CustomEvent) => {
      ...
      if (itemExists) {
        ...
        // Auto-close the history panel after selection so the freshly
        // inserted history is immediately visible in the conversation view.
        handleHistoryClose();
      }
    },
  ...
  const handlePinToTop = useCallback(
    (itemId: string) => {
      ...
      if (itemToPin !== undefined) {
        ...
        // The item unmounts and remounts under a new section, so focus must
