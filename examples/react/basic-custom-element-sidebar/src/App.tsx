...
/**
 * Example: Carbon AI Chat — Custom element (Sidebar) (React)
 *
 * Demonstrates: rendering Carbon AI Chat as a docked side panel by mounting
 * `ChatCustomElement` inside a host `<div>` that CSS sizes to a 320px sidebar.
 * A host header bar carries a toggle button that opens and closes the panel;
 * the `VIEW_CHANGE` / `VIEW_PRE_CHANGE` lifecycle hooks drive the slide-in and
 * slide-out animation so the chat only unmounts once the CSS transition ends.
 *
 * APIs exercised:
 ...
 *
 * Start reading at: the `config` constant below, then `App()` and its
...
function App() {
  ...
  // Build the host className: the closing modifier wins while the animation
  // plays, otherwise the closed modifier applies once the panel is shut.
  ...
  return (
    <>
      <header className="app-header">
        <h1 className="app-header__title">Custom Element Sidebar Example</h1>
        {instance && (
          <button
            type="button"
