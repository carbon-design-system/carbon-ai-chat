...
- Running `@carbon/ai-chat` React components under Jest with the default jsdom
  environment.
- Asserting the `cds-aichat-react` custom element mounts via
  `container.querySelector`.
- Rendering `ChatContainer` with an inline `customSendMessage` stub.
- Using `renderWriteableElements.headerBottomElement` with a `data-testid` to
  confirm React-rendered slot content.

## When to use this pattern

- You only need to assert that the chat mounts correctly (no shadow-DOM
  introspection).
...
(Use
`npm run test:watch --workspace=@carbon/ai-chat-examples-react-tests-jest-jsdom`
for watch mode.)
