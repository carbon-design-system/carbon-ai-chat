...
/**
 * Shared test fixtures and timing constants for the Vitest suite.
 *
 * Verifies (indirectly): the markdown/table/code-snippet path and the
 * conversational-search response shape, plus the timeout budgets used by
 * the helpers and `ChatContainer.test.tsx`. Adjust `WAIT_FOR_TIMEOUT` /
 * `TEST_TIMEOUT` here when tuning suite-wide patience.
 */
