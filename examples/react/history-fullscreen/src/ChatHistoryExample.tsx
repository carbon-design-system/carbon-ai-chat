...
/**
 * Writeable history-panel renderer for the chat-history-fullscreen example.
 *
 * Demonstrates: a custom React panel composed from `@carbon/ai-chat-components`
 * history primitives, wired to pin/unpin/rename/delete actions, search,
 * and selection that drives the parent `loadChat` callback.
 *
 * APIs exercised:
 *   - `HistoryShell`, `HistoryHeader`, `HistoryToolbar`, `HistoryContent`
 *   - `HistoryPanel`, `HistoryPanelMenu`, `HistoryPanelItem`, `HistoryPanelItems`
 *   - `HistorySearchItem`, `HistoryDeletePanel`
 *   - `instance.customPanels.getPanel(PanelType.HISTORY)`
 *   - `focusElementAfterRepaint` (focus restoration after DOM reorder)
 *
 * Start reading at: `ChatHistoryExample()` and the `handleSelectChat` callback.
 */
...
function ChatHistoryExample({
  ...
  const handleHistoryClose = useCallback(() => {
    // closing the writeable history panel goes through the chat instance's panel registry rather than local state, so the chat re-flows correctly when the responsive breakpoint changes.
  ...
  const handleSelectChat = useCallback(
    (event: CustomEvent) => {
      ...
      if (itemExists) {
        ...
        // on mobile the history panel covers the conversation, so auto-close after selection so the user can immediately read the loaded transcript.
        if (isMobile === true) {
          handleHistoryClose();
        }
      }
  ...
  const handlePinToTop = useCallback(
    (itemId: string) => {
      ...
      if (itemToPin !== undefined) {
        ...
        // the row is unmounted from one section and remounted in another, so focus is reattached after the next paint to keep keyboard navigation on the same logical item.
  ...
  const handleUnpin = useCallback(
    (itemId: string) => {
      ...
      if (itemToUnpin !== undefined) {
        ...
        setRegularItems((prev) => {
          // the fixture data is anchored to "Feb 10, 7:30 PM" so unpinned items land in the matching Today/Yesterday/Previous bucket regardless of the real wall-clock time.
