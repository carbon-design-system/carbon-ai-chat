 ...
 * `<cds-aichat-custom-element>` inside a host `<div>` that CSS sizes to a 320px
 * sidebar. A host header bar carries a toggle button that opens and closes the
 * panel; the `VIEW_CHANGE` / `VIEW_PRE_CHANGE` bus events drive the slide-in
 * and slide-out animation so the chat only unmounts once the CSS transition
 * has finished.
 *
 * APIs exercised:
