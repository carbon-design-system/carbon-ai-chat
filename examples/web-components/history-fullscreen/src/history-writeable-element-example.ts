...
/**
 * Custom history panel rendered into the `historyPanelElement` writeable
 * element slot.
 *
 * Demonstrates: composing the `cds-aichat-history-*` primitives into a
 * full-featured panel — pin/unpin, rename, delete confirmation, search,
 * mobile close affordance, and accessible focus restoration.
 *
 * APIs exercised:
 *   - `cds-aichat-history-shell` and friends from `@carbon/ai-chat-components`
 *   - `instance.customPanels.getPanel(PanelType.HISTORY)?.close()`
 *   - `instance.getState().customPanels.history.isMobile`
 *   - `focusElementAfterRepaint` for post-mutation focus restoration
 *
 * Start reading at: `HistoryWriteableElementExample.render`, then the
 * `_handleSelectChat` and `_handleHistoryItemAction` event handlers.
 */
...
export class HistoryWriteableElementExample extends LitElement {
  ...
  firstUpdated() {
    // Listening on window resize lets the panel react even before the chat dispatches a STATE_CHANGE, smoothing the transition during fast resizes.
    window.addEventListener("resize", this._handleResize);
  }

  _handleResize = () => {
    // Pull the mobile flag from the chat's authoritative state instead of measuring window width directly so the breakpoint matches the chat's own breakpoint logic.
    const newIsMobile = this.instance.getState().customPanels.history.isMobile;
  ...
  _handleSelectChat = (event: CustomEvent) => {
    ...
    if (itemExists) {
      ...
      if (this.isMobile === true) {
        // On mobile the panel takes the full viewport, so it must auto-close after a selection or the user would never see the loaded conversation.
  ...
  _handlePinToTop = (itemId: string) => {
    ...
    if (itemToPin !== undefined) {
      ...
      // The DOM node is destroyed and recreated when moving sections, so focus must be re-applied after Lit commits the next render.
  ...
  _handleUnpin = (itemId: string) => {
    ...
    if (itemToUnpin !== undefined) {
      ...
      // Replace with a real production implementation. The "now" anchor is hard-coded so the demo's static timestamps land in deterministic sections; production code would use `new Date()`.
      ...
      // Same rationale as pinning: the moved item is a fresh DOM node, so focus must be re-targeted after the next paint to preserve keyboard context.
      focusElementAfterRepaint(
        this.renderRoot,
        `cds-aichat-history-panel-item#${CSS.escape(itemId)}`,
      );
  ...
  _handleNewChat = () => {
    // Replace with a real production implementation. A real app would call its conversation API and then load the new transcript; the alert keeps the demo self-contained.
    window.alert("Creating new chat");
  };

  _handleHistoryClose = () => {
    // Calling close on the resolved panel is the only way the writeable element can dismiss itself; the chat owns the panel's open/closed state.
