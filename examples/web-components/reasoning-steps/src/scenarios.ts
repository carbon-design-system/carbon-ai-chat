...
function createShellMessage(
  ...
  // the shell record before any reasoning updates arrive. Without this preamble,
  // later `partial_response.message_options` updates would have no message to
  // attach to.
  instance.messaging.addMessageChunk({
    partial_item: {
...
async function streamText(
  ...
  // the response would leave residual chunks streaming in after cancellation.
  const abortHandler = () => {
    isCanceled = true;
