...
- Wrapping the editor in a `<cds-aichat-card>` so it reads as a contained
  region, with an `Esc to exit code editor` hint in the card's `footer` slot —
  styled by Carbon's `cds--form__helper-text` class, no custom CSS.
...
- Rendering a read-only `<cds-aichat-code-snippet>` inside the sent user message
  bubble with `renderUserDefinedInputNode`.
