 ...
 *
 * Demonstrates: returning a deterministic alternating sequence of
 * `MessageRequest` and `MessageResponse` items wrapped as `HistoryItem`
 * entries so the chat can rehydrate a conversation on cold start.
 *
 * APIs exercised:
...
async function customLoadHistory(_instance: ChatInstance, count = 20) {
  // Force an odd length so the conversation ends on a response item rather
  // than dangling on a user request, which matches a realistic history shape.
  const adjustedCount = count % 2 === 0 ? count + 1 : count;

  const history: HistoryItem[] = [];
  for (let i = 0; i < adjustedCount; i++) {
    // Even indices are responses; odd indices are user requests so the
    // alternation reads as response/request/response/request from the top.
