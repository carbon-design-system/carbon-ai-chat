...
class ChatActionsImpl {
  ...
  updateRawInputValue(updater: (previous: string) => string) {
    ...
    if (ref && editor) {
      ...
      if (!isPlainTextDoc(currentJson)) {
        throw new Error(
          "ChatInstance.input.updateRawValue cannot be used while the input contains non-text nodes or marked text. Use setContent instead.",
        );
      }
      if (!hasWarnedAboutUpdateRawValueDeprecation) {
        hasWarnedAboutUpdateRawValueDeprecation = true;
        consoleWarn(
          "ChatInstance.input.updateRawValue is deprecated. Use setContent.",
        );
      }
  ...
  /**
   * Replace the input content with a JSONContent doc (or a plain string,
   * or with the result of a reducer over the current content). Throws
   * when the input is not currently rendered.
   */
  setInputContent(
  ...
  /**
   * Insert content at the cursor or at `options.at` (a PM document
   * offset). Throws when the input is not currently rendered.
   */
  insertInputContent(
    ...
    ref.insertContent(content, options);
  }
