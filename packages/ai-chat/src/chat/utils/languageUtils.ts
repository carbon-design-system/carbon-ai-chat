...
/**
 * The dayjs locale files are published as UMD modules and these dynamic imports are left external by our build, so
 * the shape that comes back is decided by the consuming bundler. Most wrap the locale in a `{ default }` namespace;
 * some interop layers nest it further. {@link unwrapLocaleModule} peels those wrappers back off.
 */
const localeLoaders = {
...
/**
 * Unwraps the `default` properties a bundler may have wrapped a dynamically imported locale in until it reaches the
 * locale object itself.
 */
function unwrapLocaleModule(module: any): ILocale {
  let resolved = module;
  while (
    resolved &&
...
async function loadLocale(requestedLocale: string): Promise<ILocale> {
  try {
    ...
    const localeModule = unwrapLocaleModule(await localeLoaders[localeKey]());
    if (localeModule) {
      return localeModule;
    }
    consoleError(
