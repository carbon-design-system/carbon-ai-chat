...
<table style={{width: '100%'}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Individual chat conversation item with title, selection state, and actions menu. Supports inline rename with optional validation via <code>rename-invalid</code> and <code>rename-invalid-message</code>.</td>
    </tr>
    <tr>
    ...
    <cds-aichat-history-panel aria-label="Search results">
      <cds-aichat-history-panel-items>
        <cds-aichat-history-panel-menu expanded title="Search results">
          <cds-aichat-history-search-item name="Here's the onboarding doc that includes all the information to get started." date="Monday, 12:04 PM">
          </cds-aichat-history-search-item>
          <cds-aichat-history-search-item name="Let's use this as the master invoice document." date="Monday, 12:04 PM">
          </cds-aichat-history-search-item>
          <cds-aichat-history-search-item name="Noticed some discrepancies between these two files." date="Monday, 12:04 PM">
          </cds-aichat-history-search-item>
          <cds-aichat-history-search-item name="Do we need a PO number on every documentation here?" date="Monday, 12:04 PM">
          </cds-aichat-history-search-item>
        </cds-aichat-history-panel-menu>
      </cds-aichat-history-panel-items>
    </cds-aichat-history-panel>
...
<script>
  ...
  document.addEventListener('history-item-action', (event) => {
    ...
    console.log('Item Name:', event.detail.itemName);
...
<table style={{width: '100%'}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Bubbles up from the inner input on every keystroke while rename mode is active. Use this to validate the value and set <code>rename-invalid</code> / <code>rename-invalid-message</code> on the item.</td>
      <td>
...
// Validate the rename input on each keystroke
document.addEventListener('history-panel-item-input-change', (event) => {
  const { value, itemId } = event.detail;
  const item = document.querySelector(`cds-aichat-history-panel-item[id="${CSS.escape(itemId)}"]`);
  if (!item) return;

  const invalid = value.length > 75;
  item.setAttribute('rename-invalid', invalid ? '' : null);
  item.setAttribute('rename-invalid-message', invalid ? 'Title cannot exceed 75 characters.' : '');
});

// Persist the new name on save
document.addEventListener('history-panel-item-input-save', (event) => {
...
<table style={{width: '100%'}}>
  ...
  <tbody>
    <tr>
      <td><code>history-panel-item-input-change</code></td>
      <td>Fired on every keystroke. Use this to validate the value and reflect the result back via the <code>rename-invalid</code> and <code>rename-invalid-message</code> attributes on the parent <code>{'<cds-aichat-history-panel-item>'}</code>.</td>
      <td>
        • <code>value</code> (string) - The current input value<br/>
        • <code>itemId</code> (string) - The ID of the item being renamed
      </td>
...
**Example:**
```javascript
// Validate on each keystroke and reflect state back to the item
document.addEventListener('history-panel-item-input-change', (event) => {
  const { value, itemId } = event.detail;
  const item = document.querySelector(`cds-aichat-history-panel-item[id="${CSS.escape(itemId)}"]`);
  if (!item) return;
...
// Nothing to do on cancel — the item resets its own rename state
document.addEventListener('history-panel-item-input-cancel', () => {
