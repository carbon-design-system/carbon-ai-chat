...
export class PromptLineElement extends LitElement {
  ...
  /** Test id, applied to the editor host element. */
  ...
  override updated(changed: Map<string | number | symbol, unknown>): void {
    ...
    if (changed.has("testId") && this._editorHost) {
      if (this.testId) {
        this._editorHost.setAttribute("data-testid", this.testId);
      } else {
        this._editorHost.removeAttribute("data-testid");
      }
    }
  ...
  private _mountEditor(): void {
    ...
    if (this.testId) {
      host.setAttribute("data-testid", this.testId);
    }
