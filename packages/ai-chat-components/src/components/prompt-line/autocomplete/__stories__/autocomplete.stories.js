...
export default {
  ...
  argTypes: {
    ...
    enableDirectSend: {
      control: 'boolean',
      description:
        'When `true` (default), clicking an item fires `cds-aichat-autocomplete-send` and shows a send icon on hover. When `false`, clicking fires `cds-aichat-autocomplete-select` (insert-into-editor) and no send icon is shown.',
    },
    attached: {
      control: 'boolean',
      description:
...
export const Default = {
  render: ({ inputText, enableDirectSend, attached }) => {
    ...
    return html`
      <div style="width: 320px;">
        <cds-aichat-autocomplete
          ...
          .items=${filteredItems}
          ?attached=${attached}
          ?enable-direct-send=${enableDirectSend}
