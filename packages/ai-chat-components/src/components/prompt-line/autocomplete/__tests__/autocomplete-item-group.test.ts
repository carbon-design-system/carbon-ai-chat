...
describe('cds-aichat-autocomplete-item-group', () => {
  ...
  it('should render with title and items', async () => {
    const el = await fixture<AutocompleteItemGroupElement>(html`
      <cds-aichat-autocomplete-item-group
        title="Test Group"
        .items="${mockItems}"></cds-aichat-autocomplete-item-group>
    `);
  ...
  it('should pass enableSendButton to all items', async () => {
    const el = await fixture<AutocompleteItemGroupElement>(html`
      <cds-aichat-autocomplete-item-group
        title="Test Group"
        .items="${mockItems}"
        .enableSendButton="${false}"></cds-aichat-autocomplete-item-group>
    `);
  ...
  it('should render nothing when items array is empty', async () => {
    const el = await fixture<AutocompleteItemGroupElement>(html`
      <cds-aichat-autocomplete-item-group
        title="Empty Group"
        .items="${[]}"></cds-aichat-autocomplete-item-group>
    `);
  ...
  it('should mark item as active when focusedIndex matches', async () => {
    const el = await fixture<AutocompleteItemGroupElement>(html`
      <cds-aichat-autocomplete-item-group
        ...
        .items="${mockItems}"
        .startIndex="${0}"
        .focusedIndex="${1}"></cds-aichat-autocomplete-item-group>
    `);
