...
function ensureSheet(): CSSStyleSheet | null {
  ...
  try {
    ...
    if (typeof (candidate as any).replaceSync !== "function") {
      ...
      // the constructable-stylesheet write methods. Treat as unavailable.
      return null;
    }
    sheet = candidate;
  } catch {
...
function rewrite(): void {
  ...
  const css = Array.from(declarationsBySelector.entries())
...
function adoptOnRoot(root: Document | ShadowRoot): void {
  ...
  // adoptedStyleSheets; degrade gracefully.
