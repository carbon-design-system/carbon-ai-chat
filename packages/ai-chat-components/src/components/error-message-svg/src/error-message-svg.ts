...
/**
 * Error message SVG component.
 * Renders an error message SVG to indicate an error has occurred.
 * Supports both dark and light theme variants.
 *
 * @element cds-aichat-error-message-svg
 */
@carbonElement(`${prefix}-error-message-svg`)
class CDSAIChatErrorMessageSVG extends LitElement {
  /**
  ...
  render() {
    ...
    const id = this._linearGradientID;

    const darkGradients = svg`
      <linearGradient
        id="${id}-1"
        x1="38.91"
        y1="4.92"
...
declare global {
  interface HTMLElementTagNameMap {
    "cds-aichat-error-message-svg": CDSAIChatErrorMessageSVG;
  }
}

export { CDSAIChatErrorMessageSVG };
export default CDSAIChatErrorMessageSVG;
