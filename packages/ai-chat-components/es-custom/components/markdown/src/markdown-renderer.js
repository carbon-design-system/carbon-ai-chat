...
import '../../code-snippet/src/code-snippet-card.js';
...
import { extractTableData, DEFAULT_PAGINATION_SUPPLEMENTAL_TEXT, DEFAULT_PAGINATION_STATUS_TEXT } from './utils/table-helpers.js';
...
function renderTokenTree(node, options) {
    ...
    if (token.type === "fence") {
        ...
        const { highlight = true, feedback, showLessText, showMoreText, tooltipContent, getLineCountText = defaultLineCountText, } = options;
    ...
    if (sanitize && !isCustomElement) {
        attrs = Object.fromEntries(Object.entries(rawAttrs).filter(([key, value]) => {
    ...
    if (children.length === 1 && children[0].token.type === "text") {
        ...
        content = html `${repeat(normalizedChildren, (c, index) => {
...
function renderWithStaticTag(tag, token, content, attrs, options, _context, node) {
    ...
    const hasTaskListItems = (listNode) => !!listNode?.children?.some((child) => {
    ...
    switch (tag) {
        ...
        case "ul": {
          ...
          ${content}
            ...
            return html `<p>
        <cds-custom-unordered-list ?nested=${nested} ${spread(attrs)}>
          ${content}
        ...
        case "ol": {
          ...
          ${content}
            ...
            return html `<p>
        <cds-custom-ordered-list native ?nested=${nested} ${spread(attrs)}>
          ${content}
        ...
        case "li": {
            ...
            return html `<cds-custom-list-item ${spread(attrs)}>${content}</cds-custom-list-item>`;
        }
        case "cds-custom-checkbox": {
            const { checked, disabled, ...otherAttrs } = attrs;
            const isChecked = checked === "true";
            const isDisabled = disabled === undefined ? true : disabled === "" || disabled === "true";
            return html `<cds-custom-checkbox
        ?checked=${isChecked}
        ?disabled=${isDisabled}
        ${spread(otherAttrs)}
        >${content}</cds-custom-checkbox
      >`;
        }
        // Inline formatting
        case "strong":
        ...
        case "table": {
            ...
            const { streaming, context: parentContext, filterPlaceholderText, previousPageText, nextPageText, itemsPerPageText, downloadLabelText, locale, getPaginationSupplementalText, getPaginationStatusText, } = options;
            ...
            const renderCellTokens = (tokens, contextOverrides = {}) => html `${repeat(tokens, (child, index) => `cell-${index}:${child.token.type}:${child.token.tag}`, (child, index) => renderTokenTree(child, {
                ...options,
                context: {
                    ...options.context,
                    ...contextOverrides,
