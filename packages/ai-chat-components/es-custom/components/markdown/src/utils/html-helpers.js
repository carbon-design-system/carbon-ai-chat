...
const INLINE_HTML_ALLOWED_TOKEN_TYPES = new Set([
    "html_inline",
    "text",
    "softbreak",
    "hardbreak",
...
function parseHtmlInlineTag(content) {
    ...
    const selfClosing = trimmed.endsWith("/>") || SELF_CLOSING_HTML_TAGS.has(tagName);
...
function combineConsecutiveHtmlInline(children) {
    ...
    for (let index = 0; index < children.length; index++) {
        ...
        const stack = [openingTag.tagName]; // Track nested openings we must close.
        const chunkTokens = [startNode]; // Collect tokens to merge if the stack clears.
...
function serializeInlineToken(tokenTree) {
...
export { combineConsecutiveHtmlInline };
