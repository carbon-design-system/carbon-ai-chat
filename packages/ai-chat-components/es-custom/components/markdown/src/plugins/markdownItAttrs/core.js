...
import { get, isArrayOfObjects, last, isArrayOfFunctions, escapeRegExp } from './utils.js';
...
function getAttrs(str, start, options) {
    ...
    for (let characterIndex = start + options.leftDelimiter.length; characterIndex < str.length; characterIndex++) {
        // Check if we've reached the right delimiter (end of attributes)
        if (str.slice(characterIndex, characterIndex + options.rightDelimiter.length) === options.rightDelimiter) {
            if (key !== "") {
    ...
    if (options.allowedAttributes && options.allowedAttributes.length) {
        return attrs.filter((attrPair) => {
            ...
            return options.allowedAttributes.some((allowedAttribute) => allowedAttribute instanceof RegExp
...
function addAttrs(attrs, token) {
    for (let attributeIndex = 0; attributeIndex < attrs.length; attributeIndex += 1) {
        ...
        switch (where) {
            ...
            case "end":
                ...
                start = str.lastIndexOf(options.leftDelimiter);
                    ...
                    start === -1
                        ? -1
                        : str.indexOf(options.rightDelimiter, start + rightDelimiterMinimumShift);
 ...
 */
function getMatchingOpeningToken(tokens, closingTokenIndex) {
    ...
    const expectedType = tokens[closingTokenIndex].type.replace("_close", "_open");
    for (let searchIndex = closingTokenIndex; searchIndex >= 0; searchIndex--) {
        if (tokens[searchIndex].type === expectedType &&
 ...
 */
function test(tokens, baseIndex, rule) {
    const result = {
        match: false,
        j: null,
    };
    // Calculate the actual token index using shift (relative) or position (absolute)
    const targetIndex = rule.shift !== undefined ? baseIndex + rule.shift : rule.position;
    ...
    for (const key of Object.keys(rule)) {
        ...
        if (key === "children" && isArrayOfObjects(rule.children)) {
            ...
            if (childTests.every((childRule) => childRule.position !== undefined)) {
                match = childTests.every((childRule) => test(children, childRule.position, childRule).match);
                if (match) {
                    // Calculate the actual index (handling negative positions)
                    const childPosition = last(childTests).position;
                    result.j =
                        childPosition >= 0
                            ? childPosition
                ...
                for (let childIndex = 0; childIndex < children.length; childIndex++) {
                    match = childTests.every((childRule) => test(children, childIndex, childRule).match);
                    if (match) {
                        result.j = childIndex;
                        break;
                    }
...
export { addAttrs, getAttrs, getMatchingOpeningToken, hasDelimiters, removeDelimiter, test };
